...
public class WebClientConfig {
    ...
    private Mono<ClientResponse> sessionTokenGenerator(ClientRequest request, ExchangeFunction ex) {
        ...
        ClientRequest clientRequest = ClientRequest.from(request)
                .headers(h -> h.setBasicAuth("some-user", "some-password"))
                .build();
    ...
    private Mono<ClientResponse> sessionToken(ClientRequest request, ExchangeFunction ex) {
        System.out.println("generating session token");
        ClientRequest clientRequest = ClientRequest.from(request)
                .headers(h -> h.setBearerAuth("some-length"))
                .build();
        return ex.exchange(clientRequest);
    }
    */
    ...
    private ClientRequest withBasicAuth(ClientRequest request) {
        return  ClientRequest.from(request)
                .headers(h -> h.setBasicAuth("username", "passoword"))
                .build();
    }

    private ClientRequest withOAuth(ClientRequest request) {
        return ClientRequest.from(request)
                .headers( httpHeaders -> httpHeaders.setBasicAuth("some-token"))
                .build();
    }
}
